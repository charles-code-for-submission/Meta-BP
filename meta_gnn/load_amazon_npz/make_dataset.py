...
from .preprocess import to_binary_bag_of_words, remove_underrepresented_classes, \
    eliminate_self_loops, binarize_labels, add_self_loops
from .util import is_binary_bag_of_words


# todo only this one is used
def get_dataset(name, data_path, standardize, rm_loop, add_loop,  _log, train_examples_per_class=None, val_examples_per_class=None):
    if _log is not None:
    ...
    if rm_loop:
        graph_adj = eliminate_self_loops(graph_adj)
    if add_loop:
        graph_adj = add_self_loops(graph_adj)
